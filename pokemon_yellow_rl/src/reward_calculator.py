"""Reward Calculator - Pokemon Yellow RL with Floor is Lava, Battle Rewards, HM Detection"""
import json
import time
...
class RewardCalculator:
    ...
    def calculate_reward(self, prev_state: Dict, curr_state: Dict, return_breakdown: bool = False):
        ...
        # Dynamic gravity curriculum (overrides all other rewards when enabled)
            ...
            for slot in range(min(6, curr_state.get('party_count', 0))):
                ...
                if curr_hp_ratio < self.battle_lowest_hp[slot]:
                    ...
                    if curr_hp_ratio < 0.5:
                        print(f"  {C.FIRE}🌡️ [HOT/COLD]{C.RESET} Pokemon {slot+1} HP dropping! {C.RED}({int(curr_hp_ratio*100)}%){C.RESET} - Healing reward {C.FIRE}increasing!{C.RESET}")
                ...
                if prev_status > 0:
                    ...
                    print(f"  {C.YELLOW}⚠️ [STATUS]{C.RESET} Pokemon {slot+1} afflicted with {C.RED}{status_name}{C.RESET}! Cure reward building...")
